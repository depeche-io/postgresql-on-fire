...
We can also see a current status of a WAL receiver process.
<details><summary>Solution</summary>
<br />
...
Or in a more granular manner.

<details><summary>Solution</summary>
<br />

```plain
SELECT pg_is_wal_replay_paused(), pg_last_wal_receive_lsn(), pg_last_wal_replay_lsn(), pg_last_xact_replay_timestamp();
```{{exec}}
</details>

There are [4 different LSNs](https://www.cybertec-postgresql.com/en/monitoring-replication-pg_stat_replication/) in fact:
* `sent_lsn` / `receive_lsn`: How much WAL has been sent over the network already?
